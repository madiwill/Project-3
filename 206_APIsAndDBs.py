...
## The names of anyone you worked with on this project: Mallory Robbins, JD 
...
def get_user_tweets(username):
	identifier = "twitter_{}".format(username)
	if identifier in CACHE_DICTION:
		print('using cached data', username)
		results = CACHE_DICTION[identifier]
	else:
		print('getting data from internet', username)
		results = api.user_timeline(id = username)
		CACHE_DICTION[identifier] = results
		f=open(CACHE_FNAME, 'w')
		f.write(json.dumps(CACHE_DICTION))
		f.close()

	return CACHE_DICTION[identifier]
...
## user id or screenname you can find all the info you want about 
## the user.
...
statement += 'Tweets (tweet_id INTEGER PRIMARY KEY, text TEXT, time_posted TIMESTAMP, user_id TEXT NOT NULL, retweets INTEGER, FOREIGN KEY (user_id) REFERENCES Users(user_id) on UPDATE SET NULL)'
cur.execute(statement)

for tweet in umich_tweets:
	tup = tweet['id_str'], tweet['text'], tweet['user']['id_str'], tweet['created_at'], tweet['retweet_count']
	cur.execute('INSERT OR IGNORE INTO Tweets (tweet_id, text, user_id, time_posted, retweets) VALUES (?, ?, ?, ?, ?)', tup)
...
for user in umich_tweets:
	for info in user['entities']['user_mentions']:
		user = api.get_user(id=info['id_str'])
		tup = user['id_str'], user['screen_name'], user['favourites_count'], user['description']
...
# for @umich user
temp_user = "umich"
identifier = "user_{}".format(temp_user)
if identifier in CACHE_DICTION:
	my_var = CACHE_DICTION[identifier]
else:
	my_var = api.get_user(temp_user)
	CACHE_DICTION[identifier] = my_var
	f=open(CACHE_FNAME, 'w')
	f.write(json.dumps(CACHE_DICTION))
	f.close()	

z = 'INSERT OR IGNORE INTO Users VALUES (?, ?, ?, ?)'
cur.execute(z, ((my_var['id_str'], my_var['screen_name'], my_var['favourites_count'], my_var['description'])))
...
all_screennames_fetched = cur.fetchall()
screen_names = []
for name in all_screennames_fetched:
...
cur.execute('SELECT Users.screen_name, Tweets.text FROM Tweets INNER JOIN Users ON Tweets.user_id = Users.user_id ORDER BY Tweets.retweets DESC')
